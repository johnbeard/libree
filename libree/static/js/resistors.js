...
define(function() {
    ...
    var Resistors = function () {};
    
    // Thrown when a bad tolerance is requested
    Resistors.BadToleranceException = function(tol) { this.tol = tol};
    ...
    // mapping between tolerance and resistor series
    eiaTol = { 20: 6, 10: 12, 5:24, 2:48, 1:96, 0.5:192, 0.25:192, 0.1:192};
    
    /*! round a value to the nearest preferred value
    * @param resistance nominal value to round
    * @param tolSeries tolerance (in percent) or EIA series to select from
    * @param metric choose nearest based on "arithemetic" "geometric" series
    * @param rounding rounding method: "up" "down" or "nearest" (ties up)
    */
    Resistors.round = function(resistance, tolSeries, metric, rounding) {
        
        //we were given a tolerance, map to series
        if (tolSeries in eiaTol) {
            tolSeries = eiaTol[tolSeries];
        };
        
        //check we know about this series
        if (!(tolSeries in eiaValues))
            throw new Resistors.BadToleranceException(tolSeries);
